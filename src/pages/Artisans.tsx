...
export default function Artisans() {
  ...
  const ticketsArtisan = tickets.filter(t => t.status === "recherche_artisan");
  ...
  return (
    <div className="space-y-6 max-w-5xl">
      <div><h1 className="text-xl font-bold font-display">Artisans</h1><p className="text-sm text-muted-foreground">Recherche et gestion des prestataires</p></div>
      <Tabs defaultValue="tickets">
        ...
        <TabsContent value="tickets" className="space-y-3 mt-4">
          {ticketsArtisan.length === 0 ? <Card className="border-0 shadow-[0_20px_60px_-10px_hsl(180_5%_11%/0.06)]"><CardContent className="py-12 text-center text-muted-foreground">Aucun ticket en recherche d'artisan</CardContent></Card> :
        ...
        <TabsContent value="annuaire" className="space-y-4 mt-4">
          ...
          <div className="grid gap-3 md:grid-cols-2">
            {filteredArtisans.map(a => (
              <Card key={a.id} className="border-0 shadow-[0_20px_60px_-10px_hsl(180_5%_11%/0.06)]"><CardContent className="p-4">
